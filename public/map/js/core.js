...
log4jsLogger.setLevel(Log4js.Level.ALL);core.Monitor=function(fadeElement,logContentElem){core.assert(fadeElement,"fadeElement can not be null");
...
};core.UserAgent={_isVMLSupported:null,isVMLSupported:function(){if(!core.Utils.isDefined()){this._isVMLSupported=navigator.appVersion.match(/MSIE (\d\.\d)/)
}return this._isVMLSupported
},isSVGSupported:function(){return !core.UserAgent.isVMLSupported()
},isMozillaFamily:function(){return this.browser=="Netscape"||this.browser=="Firefox"
...
core.UserAgent.init();core.Utils={isDefined:function(val){return val!==null&&val!==undefined
},escapeInvalidTags:function(text){return text
}};
objects={};
...
core.assert=function(assert,message){if(!assert){var stack;
try{null.eval()
}catch(e){stack=e
...
var curtop=0;
if(divElement.offsetParent){curleft=divElement.offsetLeft;
curtop=divElement.offsetTop;
while(divElement=divElement.offsetParent){curleft+=divElement.offsetLeft;
curtop+=divElement.offsetTop
}}return{x:curleft,y:curtop}
};
core.Utils.innerXML=function(node){if(node.innerXML){return node.innerXML
...
core.Utils.createDocumentFromText=function(str,mimetype){if(!mimetype){mimetype="text/xml"
}if(window.DOMParser){var parser=new DOMParser();
return parser.parseFromString(str,mimetype)
}else{if(window.ActiveXObject){var domDoc=core.Utils.createDocument();
...
},processInfo:function(){if($("lbContent")){$("lbContent").remove()
...
}},deactivate:function(time){if($("lbContent")){$("lbContent").remove()
...
if(window.LoggerService){Log4js.WiseServerAppender=function(){this.layout=new Log4js.SimpleLayout()
};
...
}window.onerror=function(sMsg,sUrl,sLine){window.hasUnexpectedErrors=true;
